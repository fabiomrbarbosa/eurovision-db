<script lang="ts">
	...
	const selectedRow = $derived(
		results.find((r) => r.country === selected) ?? null,
	);
</script>
	...
	<div class="selector">
		{#each results as row}
			<button
				...
				onclick={() =>
					(selected = selected === row.country ? null : row.country)}
			>
	...
	<div class="panel">
		{#if selected && selectedRow}
			<div class="panel-header">
				<p
					class="mono cyan"
					style="font-size:0.72rem;text-transform:uppercase;letter-spacing:0.1em"
				>
					Votes received by
				</p>
				<h3>
					<img class="flag" src={countryFlagUrl(selectedRow.country)} alt="" />
			...
			{:else}
				<p class="muted" style="padding:1rem 0">
					No detailed vote data available.
				</p>
			{/if}
		{:else}
