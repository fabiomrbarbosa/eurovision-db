<script lang="ts">
	...
	const finalSorted = $derived.by(() =>
		[...finalists].sort((a, b) => {
			switch (sortKey) {
				case "running":
					return cmp(a.running, b.running, sortDir);
				case "country":
					return cmp(a.countryName, b.countryName, sortDir);
				case "artist":
					return cmp(a.artist, b.artist, sortDir);
				case "song":
					return cmp(a.song, b.song, sortDir);
				case "total":
					return cmp(a.finalTotal, b.finalTotal, sortDir);
				case "jury":
					return cmp(a.finalJury, b.finalJury, sortDir);
				case "tele":
					return cmp(a.finalTele, b.finalTele, sortDir);
				default:
					return cmp(a.finalPlace, b.finalPlace, sortDir);
			}
		}),
	);
		...
		return [...semiRounds[activeIdx].entries].sort((a, b) => {
			switch (sortKey) {
				...
				case "running":
					return cmp(a.running, b.running, sortDir);
				case "total":
					return cmp(a.total, b.total, sortDir);
				case "jury":
					return cmp(
						a.scores.find((s) => s.name === "jury")?.points ?? null,
						b.scores.find((s) => s.name === "jury")?.points ?? null,
						sortDir,
					);
				case "tele":
					return cmp(
						a.scores.find((s) => s.name === "public")?.points ?? null,
						b.scores.find((s) => s.name === "public")?.points ?? null,
						sortDir,
					);
				default:
					if (a.place === null && b.place === null)
						return cmp(a.running, b.running, "asc");
	...
	const semiHasScores = $derived(
		!isGrandFinal &&
			semiRounds[activeIdx].entries.some((e) => e.scores.length > 0),
	);
		...
		{#if finalists.length > 0}
			<div class="table-scroll">
				<table class="results-table">
					<thead>
						<tr>
							<th
								class="col-place th-sort"
								class:sorted={sortKey === "place"}
								class:sorted-desc={sortKey === "place" && sortDir === "desc"}
								tabindex="0"
								onclick={() => sortBy("place")}
								onkeydown={(e) => onThKey(e, "place")}>#</th
							>
							<th
								class="col-run th-sort"
								class:sorted={sortKey === "running"}
							...
							<th
								class="th-sort"
								class:sorted={sortKey === "country"}
								class:sorted-desc={sortKey === "country" && sortDir === "desc"}
								tabindex="0"
								onclick={() => sortBy("country")}
								onkeydown={(e) => onThKey(e, "country")}>Country</th
							>
							<th
								class="th-sort"
								class:sorted={sortKey === "artist"}
								class:sorted-desc={sortKey === "artist" && sortDir === "desc"}
								tabindex="0"
								onclick={() => sortBy("artist")}
								onkeydown={(e) => onThKey(e, "artist")}>Artist</th
							>
							<th
								class="th-sort"
								class:sorted={sortKey === "song"}
								class:sorted-desc={sortKey === "song" && sortDir === "desc"}
								tabindex="0"
								onclick={() => sortBy("song")}
								onkeydown={(e) => onThKey(e, "song")}>Song</th
							>
							{#if hasJuryTele}
								<th
									class="right th-sort"
									class:sorted={sortKey === "jury"}
									class:sorted-desc={sortKey === "jury" && sortDir === "desc"}
									tabindex="0"
									onclick={() => sortBy("jury")}
									onkeydown={(e) => onThKey(e, "jury")}>Jury</th
								>
								<th
									class="right th-sort"
									class:sorted={sortKey === "tele"}
									class:sorted-desc={sortKey === "tele" && sortDir === "desc"}
									tabindex="0"
									onclick={() => sortBy("tele")}
									onkeydown={(e) => onThKey(e, "tele")}>Tele</th
								>
							{/if}
							<th
								class="right th-sort"
								class:sorted={sortKey === "total"}
								class:sorted-desc={sortKey === "total" && sortDir === "desc"}
								tabindex="0"
								onclick={() => sortBy("total")}
								onkeydown={(e) => onThKey(e, "total")}>Total</th
							>
						</tr>
					</thead>
					<tbody>
						{#each finalSorted as r}
							<tr class:row--winner={r.finalPlace === 1}>
								...
								{/if}
								<td class="right mono">
									<span class:pts--gold={r.finalPlace === 1}
										>{r.finalTotal ?? "—"}</span
									>
								</td>
							</tr>
			...
			{#if finalHasScores}
				<div class="breakdown-wrapper">
					<p class="breakdown-hint muted">
						Select a country to see who voted for them.
					</p>
					{#key activeIdx}
						<ScoreBreakdown
							results={finalBreakdownResults}
							{countries}
							{hasJuryTele}
						/>
					{/key}
	...
	{:else}
						...
						<tr class:row-q={entry.qualified}>
							...
							{#if semiHasJuryTele}
								<td class="right mono muted"
									>{entry.scores.find((s) => s.name === "jury")?.points ??
										"—"}</td
								>
								<td class="right mono muted"
									>{entry.scores.find((s) => s.name === "public")?.points ??
										"—"}</td
								>
							{/if}
							<td class="right mono">
								<span class:pts-gold={entry.qualified}
									>{entry.total ?? "—"}</span
								>
							</td>
							<td class="col-q">
		...
		{#if semiHasScores}
			<div class="breakdown-wrapper">
				...
				{#key activeIdx}
					<ScoreBreakdown
						results={semiBreakdownResults}
						{countries}
						hasJuryTele={semiHasJuryTele}
					/>
				{/key}
			</div>
		{/if}
...
<style>
	...
	.row-q:hover td {
		background: var(--c-surface-green-hover);
	}
	...
	.th-sort.sorted.sorted-desc::after {
		content: " ↓";
	}
